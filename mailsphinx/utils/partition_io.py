...
and mailsphinx are separate git repositories with independent release
cycles. An earlier version of this fix had pushvivid/mailsphinx reach
into sphinxval's directory at runtime (sys.path.insert + import) to use
this module live from the sphinxval repo -- that avoided sphinxval's
heavier application module graph (object_handler, classes, config's
app-level singleton, etc.) and a circular-import bug those modules are
prone to, but it was still a live cross-repo dependency: no version
pinning, invisible to each repo's own tests, and broken by the mere
directory reorganization of another team's repo (which happened once
